...
namespace slam
{
using G2OVector2d = std::vector<Eigen::Vector2d, Eigen::aligned_allocator<Eigen::Vector2d>>;
using G2OVector3d = std::vector<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d>>;
...
class VertexLandmark : public g2o::BaseVertex<3, Eigen::Matrix<double, 3, 1>>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
...
void optimize_between_frame(Frame::Ptr current, Frame::Ptr last, std::vector<cv::DMatch> &matches, 
                            std::unordered_map<unsigned long, Landmark> &landmarks, int num_ite);

void optimize_pose_between_frame(Frame::Ptr current, Frame::Ptr last, std::vector<cv::DMatch> &matches, 
                            std::unordered_map<unsigned long, Landmark> &landmarks, int num_ite);
}